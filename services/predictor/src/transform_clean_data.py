...
class TransformCleanData:
    ...
    def validate_data(
        ...
        if (prcentage_dataset_null_values > treshold_null_values):
            raise Exception(f"Dataset has too many null values: {prcentage_dataset_null_values:.2%} exceeds threshold of {treshold_null_values:.2%}")
        else:
            self.ts_data = self.ts_data.dropna()
            logger.info(f"Dropped {num_rows_df_before_clean - len(self.ts_data)} rows with null values")
