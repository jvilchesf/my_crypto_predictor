...
def generate_exploratory_data_analysis(ts_data: pd.DataFrame, output_file_path: str):
    """
    ...
    profile = ProfileReport(
        ...
        title="Technical Indicators Prediction EDA",
    )
...
def load_data_from_risingwave(
    ...
    result: pd.DataFrame = rw.fetch(query, format=OutputFormat.DATAFRAME)
...
def train(
    ...
    logger.info(
        f"Starting training for {symbol} with {days_in_past} days in the past and {candle_seconds} seconds per candle"
    )
    ...
    with mlflow.start_run(nested=True):
        logger.info("Start loggin data to MLflow")

        # 1. Fetch data from RisingWave for the given symbol and days in the past
        logger.info(
            "Loading data from RisingWave for the given symbol and days in the past"
        )
        ts_data = load_data_from_risingwave(
            rw_host, rw_port, rw_user, rw_database, symbol, days_in_past, candle_seconds
        )
        ...
        ts_data_for_profiling = (
            ts_data.head(n_rows_for_data_profiling)
            if n_rows_for_data_profiling
            else ts_data
        )
        logger.info("Profiling the data using ydata-profiling")
        generate_exploratory_data_analysis(
            ts_data_for_profiling, output_file_path="./prediction_crypto_eda.html"
        )
        # Log html to MLflow
        mlflow.log_artifact(
            local_path="./prediction_crypto_eda.html", artifact_path="eda_report"
        )

        # 5. Split it into train and test
        logger.info(
            f"Splitting data into train and test with ratio {train_test_split_ratio}"
        )

        split_data = SplitData(ts_data)
        X_train, y_train, X_test, y_test = split_data.split_train_test_datasets(
            train_test_split_ratio
        )
        X_test_compare, y_test_compare, X_test, y_test = (
            split_data.split_test_compare_datasets(X_test, y_test)
        )
        ...
        logger.info("Start Lazy predictor")
        if model_name is None:
            model_names = get_model_names(
                X_train, X_test, y_train, y_test, top_n_models, mlflow_tracking_uri
            )
        ...
        logger.info("Start selecting model")
