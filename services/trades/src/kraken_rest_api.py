...
class Kraken_Rest_API:
    def __init__(self, cryptos_id: list[str], last_n_days_rest_api: int):
    ...
    def get_trades(self) -> list[Trade]:
        ...
        try:
            ...
            loguru.logger.error("Error getting trades")
            time.sleep(10)
            raise
        ...
        if self.since_timestamp_ns > int(time.time_ns() - 30 * 1e9):
            ...
            # But if it is not the last it is necessary to re-assign cryptos_id and since_timestamp_ns
                ...
                self.cryptos_id = self.list_cryptos_id[
                    self.list_cryptos_id.index(self.cryptos_id) + 1
                ]
                self.since_timestamp_ns = int(
                    time.time_ns()
                    - self.last_n_days_rest_api * 24 * 60 * 60 * 1000000000
                )
